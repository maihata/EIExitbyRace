...
oregon_data_long <- race_oregon %>% 
  ...
  group_by(race) %>% 
  mutate(percentage = round(values/exit_total, digits = 2))
...
Preparatory work: As the original datasets from OSEP contained dependent variables that are not relevant, the following preparatory work took place:
